...
class OmissionDialog(simpledialog.Dialog):
    """Custom dialog for handling omitted period detection"""
    
    def __init__(self, parent, suggested_date):
        self.suggested_date = suggested_date
        self.interpolated_date = None
        self.user_confirmed = False
        super().__init__(parent, title="Possible Omission Detected")
    
    def body(self, frame):
        """Create dialog body"""
        # Message
        msg = tk.Label(frame, text="This cycle is unusually long.\nDid you forget to record a period?", 
                      font=("Arial", 11), pady=10)
        msg.grid(row=0, column=0, columnspan=3, pady=10)
        
        # Suggested date
        tk.Label(frame, text="Suggested date:", font=("Arial", 10)).grid(row=1, column=0, sticky="e", padx=5)
        
        # Parse suggested date
        date_obj = datetime.strptime(self.suggested_date, '%Y-%m-%d')
        
        # Day spinner
        self.day_var = tk.StringVar(value=str(date_obj.day))
        day_spinner = tk.Spinbox(frame, from_=1, to=31, width=5, 
                                textvariable=self.day_var, font=("Arial", 10))
        day_spinner.grid(row=1, column=1, padx=2)
        
        # Month spinner
        self.month_var = tk.StringVar(value=str(date_obj.month))
        month_spinner = tk.Spinbox(frame, from_=1, to=12, width=5, 
                                  textvariable=self.month_var, font=("Arial", 10))
        month_spinner.grid(row=1, column=2, padx=2)
        
        # Year spinner
        self.year_var = tk.StringVar(value=str(date_obj.year))
        year_spinner = tk.Spinbox(frame, from_=2020, to=2030, width=8, 
                                 textvariable=self.year_var, font=("Arial", 10))
        year_spinner.grid(row=1, column=3, padx=2)
        
        return day_spinner  # Initial focus
    
    def buttonbox(self):
        """Create custom buttons"""
        box = tk.Frame(self)
        
        yes_btn = tk.Button(box, text="Yes, Add It", width=12, 
                           command=self.confirm, bg="#4CAF50", fg="white", 
                           font=("Arial", 10, "bold"))
        yes_btn.pack(side="left", padx=5, pady=5)
        
        no_btn = tk.Button(box, text="No, Skip", width=12, 
    ...
    def confirm(self):
        ...
        try:
            ...
            self.interpolated_date = date_obj.strftime('%Y-%m-%d')
